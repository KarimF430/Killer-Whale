...
/**
 * Fetch models from CarDekho API (publicly available)
 * CarDekho exposes model data in their page source as JSON-LD
 */
async function fetchCarDekhoModels(brandName) {
  const brandSlug = brandName.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
  const url = `https://www.cardekho.com/${brandSlug}`;
  ...
  try {
    ...
    // Method 2: Extract from model cards
    $('.gsc_col-xs-12.gsc_col-sm-6.gsc_col-md-4, .modelList li, .carName').each((i, elem) => {
      const $elem = $(elem);
      const name = $elem.find('h3, .title, a').first().text().trim();
      const price = $elem.find('.price, .priceRange').text().trim();
      const link = $elem.find('a').first().attr('href');
      
      if (name && name.length > 2 && !name.includes('undefined')) {
        models.push({
          name: name.replace(brandName, '').trim(),
          price: price,
          url: link,
          source: 'cardekho-html'
        });
      }
    });
    ...
    const uniqueModels = Array.from(
      new Map(models.map(m => [m.name.toLowerCase(), m])).values()
    );
