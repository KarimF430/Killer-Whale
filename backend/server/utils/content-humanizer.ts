...
function neutralizePositiveLanguage(text: string): string {
    ...
    for (const { positive, regex } of POSITIVE_REGEXES) {
        result = result.replace(regex, () => getRandomItem(POSITIVE_REPLACEMENTS[positive]));
    }
...
function addContractions(text: string): string {
    ...
    for (const { formal, regex } of CASUAL_REGEXES) {
        result = result.replace(regex, () => getRandomItem(CASUAL_REPLACEMENTS[formal]));
    }
